...
func (a *phAdapter) Plan() []domain.PlannedStep {
	payMode := paymentMode(a.cfg, a.cfg.PayMongoSecretKey != "")
	return []domain.PlannedStep{
		{Seq: 1, Type: domain.StepFounderKYC, Provider: "HyperVerge (PhilID)", Title: "Verify founder identity", Mode: domain.ModeMock},
		{Seq: 2, Type: domain.StepNameCheck, Provider: "SEC name verification", Title: "Verify company name", Mode: domain.ModeMock},
		{Seq: 3, Type: domain.StepEntityReg, Provider: "SEC eSPARC / OneSEC", Title: "Register corporation (SEC)", Mode: domain.ModeMock},
		{Seq: 4, Type: domain.StepTaxReg, Provider: "BIR (Form 1903/2303)", Title: "Register with BIR (TIN)", Mode: domain.ModeMock},
		{Seq: 5, Type: domain.StepBankAccount, Provider: "UnionBank", Title: "Open corporate bank account", Mode: domain.ModeMock},
		{Seq: 6, Type: domain.StepPaymentGateway, Provider: "PayMongo", Title: "Activate payment gateway", Mode: payMode},
		{Seq: 7, Type: domain.StepCompliance, Provider: "SSS + PhilHealth + Pag-IBIG", Title: "Register as employer", Mode: domain.ModeMock},
	}
}
